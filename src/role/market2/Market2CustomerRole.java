 ...
 */
public class Market2CustomerRole extends Role implements Customer {
	private String name;
	 ...
	 */
	public Market2CustomerRole(PersonAgent p, String name){
		super(p);
		this.name = name;
	...
	/**
	 * hack to establish connection to greeter agent.
	 */
	...
	private void GoToMarket(){
		...
		greeter.msgINeedInventory(this, this.getGui().getWaitingPosX(), this.getGui().getWaitingPosY());
	}
	...
	public boolean restaurantOpen() {
		if (greeter instanceof Market2GreeterRole){
			Market2GreeterRole h = (Market2GreeterRole) greeter;
			if (h !=null && h.isActive() && h.isOpen())
				return true;
		}
		return false;
		/*
		if(greeter != null && greeter.isActive() && greeter.isOpen())
			return true;
		return false;*/
	}
